...
from conftest import RecordedSleep, ScriptedTransport, rss_source
...
def test_fetch_bytes_returns_upstream_bytes_untouched():
    ...
    assert fetch_bytes(rss_source(), get,
                       RecordedSleep()) == raw


def test_fetch_parses_the_body_with_the_source_strategy():
...
def test_a_retryable_status_is_retried_until_it_succeeds():
    get = ScriptedTransport((503, b""), (429, b""), (200, RSS))
    sleep = RecordedSleep()

    outcome = fetch(rss_source(), get, sleep)
...
def test_the_retry_budget_is_three_attempts(status):
...
def test_a_deterministic_4xx_fails_without_retrying():
...
def test_a_missing_feed_is_a_deterministic_failure():
    get = ScriptedTransport((404, b""))

    outcome = fetch(rss_source(), get, RecordedSleep())

    assert not outcome.ok and outcome.error == "HTTP 404"


def test_a_transport_failure_is_retried_and_then_reported():
...
def test_a_transport_failure_can_be_followed_by_success():
...
def test_fetch_bytes_raises_rather_than_returning_an_outcome():
    ...
    with pytest.raises(FetchError, match="HTTP 403"):
        fetch_bytes(rss_source(), get, RecordedSleep())
